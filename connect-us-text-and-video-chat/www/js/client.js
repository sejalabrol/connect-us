...
let recStartTime;
let recElapsedTime;
let roomTheme = "neon"; // neon - dark - forest - ghost 
...
function getHtmlElementsById() {
  ...
  startGameBtn = getId("startGameBtn");
  twoPlayerBtn = getId("twoPlayerBtn");
...
function setButtonsTitle() {
  ...
  tippy(startGameBtn, {
    content: "PLAY Flappy Bird",
    placement: "right-start",
  });
  tippy(twoPlayerBtn, {
    content: "PLAY Snake with a friend",
    placement: "right-start",
  });
  tippy(mySettingsBtn, {
  ...
  tippy(hideEveryoneBtn, {
    content: "Turn everyone's video off except yourself",
    placement: "top",
  });
...
/**
 * Get peer info using DetecRTC - https://github.com/muaz-khan/DetectRTC
...
function welcomeUser() {
  ...
  Swal.fire({
    ...
    html:
      `
      <br/> 
      <p style="color:white;">Share this meeting invite</p>
...
/**
 * Only one side of the peer connection should create the offer, the signaling server 
 * picks one to be the offerer. The other user will get a 'sessionDescription' event and 
 * will create an offer, then send back an answer 'sessionDescription' to us
 * @param {*} peer_id
 */
...
/**
 * Peers exchange session descriptions which contains information about their audio / video
 * settings and that sort of stuff. First the 'offerer' sends a description to the
 * 'answerer' (with type "offer"), then the answerer sends one back (with type "answer").
 * @param {*} config
 */
...
/**
 * When a user leaves a channel (or is disconnected from the signaling server) everyone
 * will recieve a 'removePeer' message telling them to trash the media channels they have
 * open for those that peer. If it was this client that left a channel, they'll also
 * receive the removePeers. If this client was disconnected, they wont receive removePeers,
 * but rather the signaling_socket.on('disconnect') code will kick in and tear down all the
 * peer sessions.
 * @param {*} config
 */
...
/**
 * Set room theme neon - dark - forest - sky - ghost
 * @param {*} theme
 */
 ...
 * Ask user for permission to use the computers microphone and/or camera, attach it to an
 * <audio> or <video> tag if they give us access.
...
/**
 * On video player click, go on full screen mode || On button click, go on full screen mode.
...
async function shareRoomUrl() {
  ...
  if (navigator.share) {
    ...
    try {
      ...
      browsers and mobile devices console.error("navigator.share", err);  */
    }
  }
 ...
 * can Record One Media Stream at one time
...
/**
 * Search peer by name in chat room lists to send private messages
 */
function searchPeer() {
 ...
 * Detect also if url is an img to create preview of it
 * @param {*} text
 * @returns html
 */
...
/**
 * Check if url passed is an image
 * @param {*} url
 * @returns true/false
 */
...
/**
 * Send message over Secure dataChannels otherwise over signaling server
 * @param {*} name
 * @param {*} toName
 * @param {*} msg
  ...
  Swal.fire({
    ...
    title:
      element == "audio"
        ...
        : "Turn everyone's video off except yourself?",
    text:
      element == "audio"
        ? "Once muted, you won't be able to unmute them, but they can unmute themselves at any time."
        : "Once their video is off, you won't be able to turn on their video, but they can at any time.",
    showDenyButton: true,
...
function kickedOut(config) {
  ...
  Swal.fire({
    ...
    background: swalBackground,
    position: "center",
    title: "You will be removed from the meet!",
    html:
      `<h2 style="color: red;">` +
      `User ` +
      peer_name +
      `</h2> will remove you from the meet in <b style="color: red;"></b> milliseconds.`,
    timer: 10000,
...
function getAbout() {
  ...
  Swal.fire({
    background: swalBackground,
    position: "center",
    title: "<a>https://connect-us-text-and-video-chat.herokuapp.com/</a>",
    imageAlt: "about",
    imageUrl: aboutImg,
    showClass: {
      popup: "animate__animated animate__fadeInDown",
